    ...
    static Object[] chooseLocalizedText(XMPNode xMPNode, String str, String str2) throws XMPException {
        if (!xMPNode.getOptions().isArrayAltText()) {
            ...
            while (iterateChildren.hasNext()) {
                ...
                if (xMPNode2.hasQualifier()) {
                    if (XMPConst.XML_LANG.equals(xMPNode2.getQualifier(1).getName())) {
    ...
    static String serializeNodeValue(Object obj) {
        ...
        if (obj == null) {
            ...
            if (obj instanceof XMPDateTime) {
                xMPDateTime = (XMPDateTime) obj;
            } else if (obj instanceof GregorianCalendar) {
                xMPDateTime = XMPDateTimeFactory.createFromCalendar((GregorianCalendar) obj);
            } else {
                str = obj instanceof byte[] ? XMPUtils.encodeBase64((byte[]) obj) : obj.toString();
            }
            str = XMPUtils.convertFromDate(xMPDateTime);
        }
        return str != null ? Utils.removeControlChars(str) : null;
    }
