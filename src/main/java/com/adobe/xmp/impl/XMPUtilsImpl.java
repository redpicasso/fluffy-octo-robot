...
public class XMPUtilsImpl implements XMPConst {
    ...
    private static void appendSubtree(XMPMetaImpl xMPMetaImpl, XMPNode xMPNode, XMPNode xMPNode2, boolean z, boolean z2) throws XMPException {
        ...
        Object obj = (!z2 || (xMPNode.getOptions().isSimple() ? !(xMPNode.getValue() == null || xMPNode.getValue().length() == 0) : xMPNode.hasChildren())) ? null : 1;
        if (!z2 || obj == null) {
    ...
    private static int classifyCharacter(char c) {
        return (SPACES.indexOf(c) >= 0 || (8192 <= c && c <= 8203)) ? 1 : COMMAS.indexOf(c) >= 0 ? 2 : SEMICOLA.indexOf(c) >= 0 ? 3 : (QUOTES.indexOf(c) >= 0 || ((12296 <= c && c <= 12303) || (8216 <= c && c <= 8223))) ? 4 : (c < ' ' || CONTROLS.indexOf(c) >= 0) ? 5 : 0;
    }
    ...
    private static boolean itemValuesMatch(XMPNode xMPNode, XMPNode xMPNode2) throws XMPException {
        ...
        if (options.getOptions() == 0) {
            return (xMPNode.getValue().equals(xMPNode2.getValue()) && xMPNode.getOptions().getHasLanguage() == xMPNode2.getOptions().getHasLanguage()) ? !xMPNode.getOptions().getHasLanguage() || xMPNode.getQualifier(1).getValue().equals(xMPNode2.getQualifier(1).getValue()) : false;
        } else {
            Iterator iterateChildren;
            XMPNode xMPNode3;
            if (!options.isStruct()) {
                ...
                while (iterateChildren.hasNext()) {
                    ...
                    if (findChildNode != null) {
                        if (!itemValuesMatch(xMPNode3, findChildNode)) {
                        }
                    }
                    return false;
                }
