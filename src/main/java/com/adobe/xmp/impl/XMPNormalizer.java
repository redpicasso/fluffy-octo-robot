    ...
    private static void migrateAudioCopyright(XMPMeta xMPMeta, XMPNode xMPNode) {
        ...
        try {
            ...
            if (findSchemaNode == null || !findSchemaNode.hasChildren()) {
                ...
                xMPMeta2.setLocalizedText("http://purl.org/dc/elements/1.1/", "rights", "", XMPConst.X_DEFAULT, stringBuilder.toString(), null);
            } else {
                int lookupLanguageItem = XMPNodeUtils.lookupLanguageItem(findSchemaNode, str);
                if (lookupLanguageItem < 0) {
                    XMPMeta xMPMeta3 = xMPMeta;
                    xMPMeta3.setLocalizedText("http://purl.org/dc/elements/1.1/", "rights", "", XMPConst.X_DEFAULT, findSchemaNode.getChild(1).getValue(), null);
                    lookupLanguageItem = XMPNodeUtils.lookupLanguageItem(findSchemaNode, str);
                }
                ...
                if (indexOf >= 0) {
                    indexOf += 2;
                    if (!str.substring(indexOf).equals(value)) {
