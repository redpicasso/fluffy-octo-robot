...
public class CompoundWrite implements Iterable<Entry<Path, Node>> {
    ...
    public static CompoundWrite fromChildMerge(Map<ChildKey, Node> map) {
        ...
        for (Entry value : map.entrySet()) {
            emptyInstance = emptyInstance.setTree(new Path((ChildKey) value.getKey()), new ImmutableTree((Node) value.getValue()));
        }
        return new CompoundWrite(emptyInstance);
    }
    ...
    public CompoundWrite addWrite(Path path, Node node) {
        ...
        if (findRootMostPathWithValue != null) {
            path = Path.getRelative(findRootMostPathWithValue, path);
            Node node2 = (Node) this.writeTree.get(findRootMostPathWithValue);
            ChildKey back = path.getBack();
            if (back != null && back.isPriorityChildName() && node2.getChild(path.getParent()).isEmpty()) {
                return this;
            }
            return new CompoundWrite(this.writeTree.set(findRootMostPathWithValue, node2.updateChild(path, node)));
        }
    ...
    public Node getCompleteNode(Path path) {
        Path findRootMostPathWithValue = this.writeTree.findRootMostPathWithValue(path);
        return findRootMostPathWithValue != null ? ((Node) this.writeTree.get(findRootMostPathWithValue)).getChild(Path.getRelative(findRootMostPathWithValue, path)) : null;
    }
    ...
    public List<NamedNode> getCompleteChildren() {
        ...
        if (this.writeTree.getValue() != null) {
            ...
            it = this.writeTree.getChildren().iterator();
            while (it.hasNext()) {
    ...
    public Map<ChildKey, CompoundWrite> childCompoundWrites() {
        Map<ChildKey, CompoundWrite> hashMap = new HashMap();
    ...
    public boolean equals(Object obj) {
        ...
        return (obj == null || obj.getClass() != getClass()) ? false : ((CompoundWrite) obj).getValue(true).equals(getValue(true));
    }
