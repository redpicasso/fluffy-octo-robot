...
final class MemoryMutationQueue implements MutationQueue {
    ...
    public void acknowledgeBatch(MutationBatch mutationBatch, ByteString byteString) {
        ...
        Assert.hardAssert(batchId == ((MutationBatch) this.queue.get(indexOfExistingBatchId)).getBatchId(), "Queue ordering failure: expected batch %d, got batch %d", Integer.valueOf(batchId), Integer.valueOf(((MutationBatch) this.queue.get(indexOfExistingBatchId)).getBatchId()));
    ...
    public MutationBatch addMutationBatch(Timestamp timestamp, List<Mutation> list, List<Mutation> list2) {
        boolean z = true;
        Assert.hardAssert(list2.isEmpty() ^ true, "Mutation batches should not be empty", new Object[0]);
    ...
    public MutationBatch getNextMutationBatchAfterBatchId(int i) {
        i = indexOfBatchId(i + 1);
        if (i < 0) {
            i = 0;
        }
        return this.queue.size() > i ? (MutationBatch) this.queue.get(i) : null;
    }
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingQuery(Query query) {
        Assert.hardAssert(query.isCollectionGroupQuery() ^ 1, "CollectionGroup queries should be handled in LocalDocumentsView", new Object[0]);
        ...
        Iterator iteratorFrom = this.batchesByDocumentKey.iteratorFrom(documentReference);
        while (iteratorFrom.hasNext()) {
            documentReference = (DocumentReference) iteratorFrom.next();
            BasePath path2 = documentReference.getKey().getPath();
    ...
    public void removeMutationBatch(MutationBatch mutationBatch) {
        ...
        this.queue.remove(0);
        ImmutableSortedSet immutableSortedSet = this.batchesByDocumentKey;
    ...
    private int indexOfExistingBatchId(int i, String str) {
        i = indexOfBatchId(i);
        boolean z = i >= 0 && i < this.queue.size();
        Assert.hardAssert(z, "Batches must exist to be %s", str);
        return i;
    }
