...
class SQLiteLruReferenceDelegate implements ReferenceDelegate, LruDelegate {
    ...
    SQLiteLruReferenceDelegate(SQLitePersistence sQLitePersistence, Params params) {
    ...
    private boolean mutationQueuesContainKey(DocumentKey documentKey) {
        return this.persistence.query("SELECT 1 FROM document_mutations WHERE path = ?").binding(EncodedPath.encode(documentKey.getPath())).isEmpty() ^ true;
    }
