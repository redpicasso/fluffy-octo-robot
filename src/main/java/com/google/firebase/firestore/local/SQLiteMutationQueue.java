...
final class SQLiteMutationQueue implements MutationQueue {
    ...
    private void writeMutationQueueMetadata() {
        this.db.execute("INSERT OR REPLACE INTO mutation_queues (uid, last_acknowledged_batch_id, last_stream_token) VALUES (?, ?, ?)", this.uid, Integer.valueOf(-1), this.lastStreamToken.toByteArray());
    }
    ...
    public MutationBatch addMutationBatch(Timestamp timestamp, List<Mutation> list, List<Mutation> list2) {
        ...
        MessageLite encodeMutationBatch = this.serializer.encodeMutationBatch(mutationBatch);
    ...
    public MutationBatch getNextMutationBatchAfterBatchId(int i) {
        i++;
        return (MutationBatch) this.db.query("SELECT batch_id, SUBSTR(mutations, 1, ?) FROM mutations WHERE uid = ? AND batch_id >= ? ORDER BY batch_id ASC LIMIT 1").binding(Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, Integer.valueOf(i)).firstValue(SQLiteMutationQueue$$Lambda$5.lambdaFactory$(this));
    }

    public List<MutationBatch> getAllMutationBatches() {
        List<MutationBatch> arrayList = new ArrayList();
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingDocumentKeys(Iterable<DocumentKey> iterable) {
        ...
        String str = "SELECT DISTINCT dm.batch_id, SUBSTR(m.mutations, 1, ?) FROM document_mutations dm, mutations m WHERE dm.uid = ? AND dm.path IN (";
        LongQuery longQuery = new LongQuery(this.db, str, Arrays.asList(new Object[]{Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid}), arrayList, ") AND dm.uid = m.uid AND dm.batch_id = m.batch_id ORDER BY dm.batch_id");
        List<MutationBatch> arrayList2 = new ArrayList();
        Set hashSet = new HashSet();
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingQuery(Query query) {
        ...
        List<MutationBatch> arrayList = new ArrayList();
        this.db.query("SELECT dm.batch_id, dm.path, SUBSTR(m.mutations, 1, ?) FROM document_mutations dm, mutations m WHERE dm.uid = ? AND dm.path >= ? AND dm.path < ? AND dm.uid = m.uid AND dm.batch_id = m.batch_id ORDER BY dm.batch_id").binding(Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, r9, prefixSuccessor).forEach(SQLiteMutationQueue$$Lambda$10.lambdaFactory$(this, arrayList, length));
        return arrayList;
    }
    ...
    public void removeMutationBatch(MutationBatch mutationBatch) {
        ...
        for (Mutation key : mutationBatch.getMutations()) {
            DocumentKey key2 = key.getKey();
            String encode = EncodedPath.encode(key2.getPath());
            this.db.execute(prepare2, this.uid, encode, Integer.valueOf(batchId));
    ...
    private MutationBatch decodeInlineMutationBatch(int i, byte[] bArr) {
        try {
            ...
            Object blobAccumulator = new BlobAccumulator(bArr);
            while (blobAccumulator.more) {
                int numChunks = (blobAccumulator.numChunks() * BLOB_MAX_INLINE_LENGTH) + 1;
                this.db.query("SELECT SUBSTR(mutations, ?, ?) FROM mutations WHERE uid = ? AND batch_id = ?").binding(Integer.valueOf(numChunks), Integer.valueOf(BLOB_MAX_INLINE_LENGTH), this.uid, Integer.valueOf(i)).first(blobAccumulator);
            }
