...
final class SQLiteQueryCache implements QueryCache {
    ...
    SQLiteQueryCache(SQLitePersistence sQLitePersistence, LocalSerializer localSerializer) {
    ...
    public void addMatchingKeys(ImmutableSortedSet<DocumentKey> immutableSortedSet, int i) {
        ...
        ReferenceDelegate referenceDelegate = this.db.getReferenceDelegate();
    ...
    public boolean containsKey(DocumentKey documentKey) {
        String encode = EncodedPath.encode(documentKey.getPath());
        return this.db.query("SELECT target_id FROM target_documents WHERE path = ? AND target_id != 0 LIMIT 1").binding(encode).isEmpty() ^ true;
    }
}
