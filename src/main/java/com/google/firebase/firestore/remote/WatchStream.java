...
import com.google.firestore.v1.ListenRequest.Builder;
...
public class WatchStream extends AbstractStream<ListenRequest, ListenResponse, Callback> {
    ...
    /* compiled from: com.google.firebase:firebase-firestore@@19.0.0 */
    interface Callback extends StreamCallback {
    ...
    WatchStream(FirestoreChannel firestoreChannel, AsyncQueue asyncQueue, RemoteSerializer remoteSerializer, Callback callback) {
        super(firestoreChannel, FirestoreGrpc.getListenMethod(), asyncQueue, TimerId.LISTEN_STREAM_CONNECTION_BACKOFF, TimerId.LISTEN_STREAM_IDLE, callback);
    ...
    public void watchQuery(QueryData queryData) {
        ...
        Builder addTarget = ListenRequest.newBuilder().setDatabase(this.serializer.databaseName()).setAddTarget(this.serializer.encodeTarget(queryData));
        Map encodeListenRequestLabels = this.serializer.encodeListenRequestLabels(queryData);
