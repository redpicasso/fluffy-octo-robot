...
public final class UserDataConverter {
    ...
    public ParsedSetData parseMergeData(Object obj, @Nullable FieldMask fieldMask) {
        ...
        for (FieldPath fieldPath : fieldMask.getMask()) {
            if (!parseAccumulator.contains(fieldPath)) {
                ...
                stringBuilder.append(fieldPath.toString());
    ...
    public ParsedUpdateData parseUpdateData(List<Object> list) {
        ...
        while (it.hasNext()) {
            FieldPath internalPath;
            Object next = it.next();
            Object next2 = it.next();
            boolean z = next instanceof String;
            boolean z2 = z || (next instanceof FieldPath);
            Assert.hardAssert(z2, "Expected argument to be String or FieldPath.", new Object[0]);
            if (z) {
            ...
            if (next2 instanceof DeleteFieldValue) {
                ...
                FieldValue convertAndParseFieldData = convertAndParseFieldData(next2, rootContext.childContext(internalPath));
                if (convertAndParseFieldData != null) {
                    rootContext.addToFieldMask(internalPath);
                    emptyObject = emptyObject.set(internalPath, convertAndParseFieldData);
                }
    ...
    private ObjectValue convertAndParseDocumentData(Object obj, ParseContext parseContext) {
        ...
        stringBuilder2.append(Util.typeName(obj));
        ...
        if (obj instanceof Timestamp) {
            ...
            if (obj instanceof DocumentReference) {
                ...
                if (documentReference.getFirestore() != null) {
                    if (!documentReference.getFirestore().getDatabaseId().equals(this.databaseId)) {
                        throw parseContext.createError(String.format("Document reference is for database %s/%s but should be for database %s/%s", new Object[]{documentReference.getFirestore().getDatabaseId().getProjectId(), documentReference.getFirestore().getDatabaseId().getDatabaseId(), this.databaseId.getProjectId(), this.databaseId.getDatabaseId()}));
                    }
                }
                ...
                throw parseContext.createError(stringBuilder.toString());
            }
        }
    }
