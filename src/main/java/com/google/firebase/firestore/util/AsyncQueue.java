...
public class AsyncQueue {
    ...
    private class DelayedStartFactory implements Runnable, ThreadFactory {
        ...
        /* synthetic */ DelayedStartFactory(AsyncQueue asyncQueue, AnonymousClass1 anonymousClass1) {
            this();
        }
    ...
    static /* synthetic */ void lambda$callTask$1(Callable callable, Executor executor, TaskCompletionSource taskCompletionSource) {
        try {
            ((Task) callable.call()).continueWith(executor, AsyncQueue$$Lambda$9.lambdaFactory$(taskCompletionSource));
        } catch (Exception e) {
    ...
    public AsyncQueue() {
        Object delayedStartFactory = new DelayedStartFactory(this, null);
            ...
            protected void afterExecute(Runnable runnable, Throwable th) {
                ...
                if (th == null && (runnable instanceof Future)) {
                    ...
                    try {
                        ...
                        if (th != null) {
                            AsyncQueue.this.lambda$new$2(th);
                        }
    ...
    public void verifyIsCurrentThread() {
        ...
        if (this.thread != currentThread) {
            throw Assert.fail("We are running on the wrong thread. Expected to be on the AsyncQueue thread %s/%d but was %s/%d", this.thread.getName(), Long.valueOf(this.thread.getId()), currentThread.getName(), Long.valueOf(currentThread.getId()));
        }
    }
    ...
    public DelayedTask enqueueAfterDelay(TimerId timerId, long j, Runnable runnable) {
        Assert.hardAssert(containsDelayedTask(timerId) ^ true, "Attempted to schedule multiple operations with timer id %s.", timerId);
    ...
    public boolean containsDelayedTask(TimerId timerId) {
        ...
        while (it.hasNext()) {
            if (((DelayedTask) it.next()).timerId == timerId) {
                return true;
            }
        }
        return false;
    ...
    static /* synthetic */ void lambda$runDelayedTasksUntil$8(AsyncQueue asyncQueue, TimerId timerId) {
        boolean z = timerId == TimerId.ALL || asyncQueue.containsDelayedTask(timerId);
        Assert.hardAssert(z, "Attempted to run tasks until missing TimerId: %s", timerId);
    ...
    private void removeDelayedTask(DelayedTask delayedTask) {
