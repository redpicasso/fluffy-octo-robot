...
public final class RSSExpandedReader extends AbstractRSSReader {
    private static final int[] EVEN_TOTAL_SUBSET = new int[]{4, 20, 52, 104, XMPError.BADSTREAM};
    ...
    public Result decodeRow(int i, BitArray bitArray, Map<DecodeHintType, ?> map) throws NotFoundException, FormatException {
        ...
        try {
            i = constructResult(decodeRow2pairs(i, bitArray));
            return i;
    ...
    private List<ExpandedPair> checkRows(List<ExpandedRow> list, int i) throws NotFoundException {
        while (i < this.rows.size()) {
            ...
            if (isValidSequence(this.pairs)) {
                ...
                try {
                    continue;
                    Collection list2 = checkRows(arrayList, i + 1);
                    return list2;
                } catch (NotFoundException unused) {
                    i++;
                }
    ...
    DataCharacter decodeDataCharacter(BitArray bitArray, FinderPattern finderPattern, boolean z, boolean z2) throws NotFoundException {
        ...
        if (Math.abs(sum - f) / f <= 0.3f) {
            ...
            for (i4 = 0; i4 < dataCharacterCounters.length; i4++) {
                ...
                if ((i4 & 1) == 0) {
                    oddCounts[i5] = i6;
                    oddRoundingErrors[i5] = f2 - ((float) i6);
                } else {
                    evenCounts[i5] = i6;
                    evenRoundingErrors[i5] = f2 - ((float) i6);
                }
            }
            ...
            for (length2 = oddCounts.length - 1; length2 >= 0; length2--) {
                if (isNotA1left(finderPattern, z, z2)) {
                    i4 += oddCounts[length2] * WEIGHTS[i][length2 * 2];
                }
                i5 += oddCounts[length2];
            }
            int i7 = 0;
            for (length2 = evenCounts.length - 1; length2 >= 0; length2--) {
                if (isNotA1left(finderPattern, z, z2)) {
                    i7 += evenCounts[length2] * WEIGHTS[i][(length2 * 2) + 1];
                }
            }
            i4 += i7;
            ...
            i = (13 - i5) / 2;
            length = SYMBOL_WIDEST[i];
            int i8 = 9 - length;
            return new DataCharacter(((RSSUtils.getRSSvalue(oddCounts, length, true) * EVEN_TOTAL_SUBSET[i]) + RSSUtils.getRSSvalue(evenCounts, i8, false)) + GSUM[i], i4);
        }
        throw NotFoundException.getNotFoundInstance();
    }
