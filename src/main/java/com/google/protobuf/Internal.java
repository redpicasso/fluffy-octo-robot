...
public final class Internal {
    ...
    public static int hashCodeByteBuffer(ByteBuffer byteBuffer) {
        ...
        if (byteBuffer.hasArray()) {
            partialHash = partialHash(byteBuffer.capacity(), byteBuffer.array(), byteBuffer.arrayOffset(), byteBuffer.capacity());
            if (partialHash == 0) {
                partialHash = 1;
            }
            return partialHash;
        ...
        duplicate.clear();
        partialHash = byteBuffer.capacity();
        while (duplicate.remaining() > 0) {
            int remaining = duplicate.remaining() <= i ? duplicate.remaining() : i;
            duplicate.get(bArr, 0, remaining);
            partialHash = partialHash(partialHash, bArr, 0, remaining);
        }
        if (partialHash == 0) {
            partialHash = 1;
        }
