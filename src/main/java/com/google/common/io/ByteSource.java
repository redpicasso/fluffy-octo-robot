...
public abstract class ByteSource {
    ...
    private static class ByteArrayByteSource extends ByteSource {
        ...
        public ByteSource slice(long j, long j2) {
            ...
            j = Math.min(j, (long) this.length);
            return new ByteArrayByteSource(this.bytes, this.offset + ((int) j), (int) Math.min(j2, ((long) this.length) - j));
        }
    ...
    private static final class ConcatenatedByteSource extends ByteSource {
        ...
        public Optional<Long> sizeIfKnown() {
            Iterable<ByteSource> iterable = this.sources;
    ...
    public long size() throws IOException {
        ...
        try {
            ...
            countBySkipping = ByteStreams.exhaust((InputStream) create.register(openStream()));
            create.close();
            return countBySkipping;
        } catch (Throwable th) {
            try {
    ...
    public byte[] read() throws IOException {
        ...
        try {
            ...
            Optional sizeIfKnown = sizeIfKnown();
            if (sizeIfKnown.isPresent()) {
                toByteArray = ByteStreams.toByteArray(inputStream, ((Long) sizeIfKnown.get()).longValue());
            } else {
