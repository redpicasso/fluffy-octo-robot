...
abstract class SmoothRateLimiter extends RateLimiter {
    ...
    static final class SmoothWarmingUp extends SmoothRateLimiter {
        ...
        void doSetRate(double d, double d2) {
            ...
            this.maxPermits = this.thresholdPermits + ((((double) j) * 2.0d) / (d2 + d3));
        ...
        long storedPermitsToWaitTime(double d, double d2) {
            ...
            if (d > 0.0d) {
                ...
                permitsToTime = (long) (((permitsToTime(d) + permitsToTime(d - min)) * min) / 2.0d);
                d2 -= min;
            } else {
                permitsToTime = 0;
            }
            return permitsToTime + ((long) (this.stableIntervalMicros * d2));
        }
