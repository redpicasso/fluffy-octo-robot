    ...
    public static final class Builder<K extends Comparable<?>, V> {
        ...
        public ImmutableRangeMap<K, V> build() {
            ...
            for (int i = 0; i < this.entries.size(); i++) {
                Object obj = (Range) ((Entry) this.entries.get(i)).getKey();
                if (i > 0) {
                    Range range = (Range) ((Entry) this.entries.get(i - 1)).getKey();
                    if (obj.isConnected(range) && !obj.intersection(range).isEmpty()) {
