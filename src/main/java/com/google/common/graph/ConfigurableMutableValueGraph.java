...
final class ConfigurableMutableValueGraph<N, V> extends ConfigurableValueGraph<N, V> implements MutableValueGraph<N, V> {
    ...
    public V putEdgeValue(N n, N n2, V v) {
        ...
        if (!allowsSelfLoops()) {
            Preconditions.checkArgument(n.equals(n2) ^ 1, "Cannot add self-loop edge on node %s, as self-loops are not allowed. To construct a graph that allows self-loops, call allowsSelfLoops(true) on the Builder.", (Object) n);
        }
