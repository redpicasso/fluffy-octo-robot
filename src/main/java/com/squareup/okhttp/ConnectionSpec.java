...
public final class ConnectionSpec {
    ...
    public List<TlsVersion> tlsVersions() {
        ...
        Object[] objArr = new TlsVersion[strArr.length];
        int i = 0;
        while (true) {
            ...
            objArr[i] = TlsVersion.forJavaName(strArr2[i]);
            i++;
        }
    }
    ...
    private ConnectionSpec supportedSpec(SSLSocket sSLSocket, boolean z) {
        String[] strArr = this.cipherSuites;
        if (strArr != null) {
            strArr = (String[]) Util.intersect(String.class, strArr, sSLSocket.getEnabledCipherSuites());
        } else {
            strArr = sSLSocket.getEnabledCipherSuites();
        }
        String[] strArr2 = this.tlsVersions;
        if (strArr2 != null) {
            strArr2 = (String[]) Util.intersect(String.class, strArr2, sSLSocket.getEnabledProtocols());
        } else {
            strArr2 = sSLSocket.getEnabledProtocols();
        }
        if (z) {
            String str = "TLS_FALLBACK_SCSV";
            if (Util.contains(sSLSocket.getSupportedCipherSuites(), str)) {
    ...
    public boolean isCompatible(SSLSocket sSLSocket) {
        ...
        strArr = this.cipherSuites;
        if (strArr == null || nonEmptyIntersection(strArr, sSLSocket.getEnabledCipherSuites())) {
            return true;
        }
        return false;
    }
