...
public class RNFetchBlobProgressConfig {
    ...
    public boolean shouldReport(float f) {
        int i = this.count;
        boolean z = false;
        Object obj = (i <= 0 || f <= 0.0f || Math.floor((double) (f * ((float) i))) > ((double) this.tick)) ? 1 : null;
        if (System.currentTimeMillis() - this.lastTick > ((long) this.interval) && this.enable && obj != null) {
            z = true;
        }
        if (z) {
            this.tick++;
