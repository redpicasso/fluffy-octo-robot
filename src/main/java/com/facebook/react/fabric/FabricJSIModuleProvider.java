...
public class FabricJSIModuleProvider implements JSIModuleProvider<UIManager> {
    ...
    public UIManager get() {
        ...
        UIManager createUIManager = createUIManager(eventBeatManager);
        Systrace.beginSection(0, "FabricJSIModuleProvider.registerBinding");
        Binding binding = new Binding();
        loadClasses();
        UIManager uIManager = createUIManager;
        binding.register(this.mJSContext, uIManager, eventBeatManager, this.mReactApplicationContext.getCatalystInstance().getReactQueueConfiguration().getJSQueueThread(), this.mComponentFactoryDelegate, this.mConfig);
