...
public class NativeAnimatedModule extends ReactContextBaseJavaModule implements LifecycleEventListener, UIManagerModuleListener {
    ...
    public void initialize() {
        ReactApplicationContext reactApplicationContext = access$300();
    ...
    private NativeAnimatedNodesManager getNodesManager() {
        if (this.mNodesManager == null) {
            this.mNodesManager = new NativeAnimatedNodesManager((UIManagerModule) access$300().getNativeModule(UIManagerModule.class));
        }
    ...
    public void startListeningToAnimatedNodeValue(final int i) {
        final AnimatedNodeValueListener anonymousClass5 = new AnimatedNodeValueListener() {
            public void onValueUpdate(double d) {
                WritableMap createMap = Arguments.createMap();
                createMap.putInt("tag", i);
                createMap.putDouble("value", d);
                ((RCTDeviceEventEmitter) NativeAnimatedModule.this.access$300().getJSModule(RCTDeviceEventEmitter.class)).emit("onAnimatedValueUpdate", createMap);
            }
        };
        this.mOperations.add(new UIThreadOperation() {
            public void execute(NativeAnimatedNodesManager nativeAnimatedNodesManager) {
                nativeAnimatedNodesManager.startListeningToAnimatedNodeValue(i, anonymousClass5);
            }
        });
    }
