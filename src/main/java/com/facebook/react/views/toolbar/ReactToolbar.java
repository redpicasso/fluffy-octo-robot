...
public class ReactToolbar extends Toolbar {
    ...
    private final Runnable mLayoutRunnable = new Runnable() {
        public void run() {
            ...
            reactToolbar.measure(MeasureSpec.makeMeasureSpec(reactToolbar.getWidth(), Ints.MAX_POWER_OF_TWO), MeasureSpec.makeMeasureSpec(ReactToolbar.this.getHeight(), Ints.MAX_POWER_OF_TWO));
    ...
    void setActions(@Nullable ReadableArray readableArray) {
        ...
        if (readableArray != null) {
            for (int i = 0; i < readableArray.size(); i++) {
                ...
                String str = PROP_ACTION_ICON;
                if (map.hasKey(str)) {
                    setMenuItemIcon(add, map.getMap(str));
                }
                str = PROP_ACTION_SHOW;
                int i2 = map.hasKey(str) ? map.getInt(str) : 0;
                String str2 = PROP_ACTION_SHOW_WITH_TEXT;
                if (map.hasKey(str2) && map.getBoolean(str2)) {
                    i2 |= 4;
                }
    ...
    private IconImageInfo getIconImageInfo(ReadableMap readableMap) {
        ...
        if (readableMap.hasKey(str)) {
            ...
            if (readableMap.hasKey(str2)) {
                return new IconImageInfo(Math.round(PixelUtil.toPixelFromDIP((float) readableMap.getInt(str))), Math.round(PixelUtil.toPixelFromDIP((float) readableMap.getInt(str2))));
            }
        }
        return null;
    }
