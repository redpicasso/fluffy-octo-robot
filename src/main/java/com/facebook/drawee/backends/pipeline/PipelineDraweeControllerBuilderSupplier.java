...
public class PipelineDraweeControllerBuilderSupplier implements Supplier<PipelineDraweeControllerBuilder> {
    ...
    public PipelineDraweeControllerBuilderSupplier(Context context, ImagePipelineFactory imagePipelineFactory, Set<ControllerListener> set, @Nullable DraweeConfig draweeConfig) {
        Supplier debugOverlayEnabledSupplier;
        ...
        if (draweeConfig != null) {
            debugOverlayEnabledSupplier = draweeConfig.getDebugOverlayEnabledSupplier();
        } else {
            debugOverlayEnabledSupplier = null;
        }
        pipelineDraweeControllerFactory.init(resources, instance, animatedDrawableFactory, instance2, bitmapMemoryCache, customDrawableFactories, debugOverlayEnabledSupplier);
        this.mBoundControllerListeners = set;
        if (draweeConfig != null) {
            imagePerfDataListener = draweeConfig.getImagePerfDataListener();
        }
        this.mDefaultImagePerfDataListener = imagePerfDataListener;
    }
