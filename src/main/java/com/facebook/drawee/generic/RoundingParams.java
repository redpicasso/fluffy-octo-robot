...
public class RoundingParams {
    ...
    public int hashCode() {
        ...
        int i = 0;
        int hashCode = (((roundingMethod != null ? roundingMethod.hashCode() : 0) * 31) + this.mRoundAsCircle) * 31;
        float[] fArr = this.mCornersRadii;
        hashCode = (((hashCode + (fArr != null ? Arrays.hashCode(fArr) : 0)) * 31) + this.mOverlayColor) * 31;
        float f = this.mBorderWidth;
        hashCode = (((hashCode + (f != 0.0f ? Float.floatToIntBits(f) : 0)) * 31) + this.mBorderColor) * 31;
        f = this.mPadding;
        if (f != 0.0f) {
            i = Float.floatToIntBits(f);
        }
        return ((((hashCode + i) * 31) + this.mScaleDownInsideBorders) * 31) + this.mPaintFilterBitmap;
    }
}
