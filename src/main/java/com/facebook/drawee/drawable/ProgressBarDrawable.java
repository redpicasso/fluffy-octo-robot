...
public class ProgressBarDrawable extends Drawable implements CloneableDrawable {
    ...
    public void draw(Canvas canvas) {
        if (!this.mHideWhenZero || this.mLevel != 0) {
            if (this.mIsVertical) {
                drawVerticalBar(canvas, PhotoshopDirectory.TAG_PRINT_FLAGS_INFO, this.mBackgroundColor);
                drawVerticalBar(canvas, this.mLevel, this.mColor);
            } else {
                drawHorizontalBar(canvas, PhotoshopDirectory.TAG_PRINT_FLAGS_INFO, this.mBackgroundColor);
    ...
    private void drawHorizontalBar(Canvas canvas, int i, int i2) {
        ...
        int width = ((bounds.width() - (this.mPadding * 2)) * i) / PhotoshopDirectory.TAG_PRINT_FLAGS_INFO;
        i = bounds.left + this.mPadding;
        int i3 = bounds.bottom - this.mPadding;
        int i4 = this.mBarWidth;
        i3 -= i4;
        this.mRect.set((float) i, (float) i3, (float) (i + width), (float) (i3 + i4));
        drawBar(canvas, i2);
    }

    private void drawVerticalBar(Canvas canvas, int i, int i2) {
        Rect bounds = getBounds();
        int height = ((bounds.height() - (this.mPadding * 2)) * i) / PhotoshopDirectory.TAG_PRINT_FLAGS_INFO;
        i = bounds.left + this.mPadding;
        int i3 = bounds.top + this.mPadding;
        this.mRect.set((float) i, (float) i3, (float) (i + this.mBarWidth), (float) (i3 + height));
        drawBar(canvas, i2);
    }
