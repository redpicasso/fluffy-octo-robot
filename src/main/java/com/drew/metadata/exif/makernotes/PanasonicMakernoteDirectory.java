...
public class PanasonicMakernoteDirectory extends Directory {
    ...
    public Face[] getRecognizedFaces() {
        ...
        try {
            ...
            while (i < uInt16) {
                int i2 = (i * 44) + 4;
                faceArr[i] = new Face(byteArrayReader.getUInt16(i2 + 20), byteArrayReader.getUInt16(i2 + 22), byteArrayReader.getUInt16(i2 + 24), byteArrayReader.getUInt16(i2 + 26), byteArrayReader.getString(i2, 20, str).trim(), Age.fromPanasonicString(byteArrayReader.getString(i2 + 28, 20, str).trim()));
                i++;
            }
            return faceArr;
        } catch (IOException unused) {
