...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public boolean executeKeyEvent(@NonNull KeyEvent keyEvent) {
        ...
        if (canScroll) {
            if (keyEvent.getAction() == 0) {
                ...
                if (keyCode == 19) {
                    ...
                    z = !keyEvent.isAltPressed() ? arrowScroll(NikonType2MakernoteDirectory.TAG_ADAPTER) : fullScroll(NikonType2MakernoteDirectory.TAG_ADAPTER);
                } else if (keyCode == 62) {
        ...
        if (isFocused() && keyEvent.getKeyCode() != 4) {
            ...
            findFocus = FocusFinder.getInstance().findNextFocus(this, findFocus, NikonType2MakernoteDirectory.TAG_ADAPTER);
            if (!(findFocus == null || findFocus == this || !findFocus.requestFocus(NikonType2MakernoteDirectory.TAG_ADAPTER))) {
                z = true;
            }
        }
        return z;
            ...
            if (actionMasked == 1) {
                ...
                if (findPointerIndex == -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        this.mLastMotionY = actionMasked - iArr2[1];
                        this.mNestedYOffset += iArr2[1];
                        if (obj != null) {
                            actionMasked = i2 - this.mScrollConsumed[1];
                            ensureGlows();
                            int i5 = scrollY + actionMasked;
                            if (i5 < 0) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowTop, ((float) actionMasked) / ((float) getHeight()), motionEvent2.getX(findPointerIndex) / ((float) getWidth()));
                                if (!this.mEdgeGlowBottom.isFinished()) {
                                    this.mEdgeGlowBottom.onRelease();
                                }
                            } else if (i5 > i3) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowBottom, ((float) actionMasked) / ((float) getHeight()), 1.0f - (motionEvent2.getX(findPointerIndex) / ((float) getWidth())));
    ...
    public boolean pageScroll(int i) {
        Object obj = i == NikonType2MakernoteDirectory.TAG_ADAPTER ? 1 : null;
    ...
    public boolean arrowScroll(int i) {
        ...
        if (findNextFocus == null || !isWithinDeltaOfScreen(findNextFocus, maxScrollAmount, getHeight())) {
            ...
            } else if (i == NikonType2MakernoteDirectory.TAG_ADAPTER && getChildCount() > 0) {
            ...
            if (i != NikonType2MakernoteDirectory.TAG_ADAPTER) {
    ...
    public void computeScroll() {
        if (!this.mScroller.isFinished()) {
            ...
            if (i2 != 0) {
                ...
                if (currY == 0 || (currY == 1 && scrollRange > 0)) {
                    obj = 1;
                }
                if (obj != null) {
                    ensureGlows();
