...
class GzipInflatingBuffer implements Closeable {
    ...
    private class GzipMetadataReader {
        ...
        private void skipBytes(int i) {
            ...
            if (access$000 > 0) {
                access$000 = Math.min(access$000, i);
                GzipInflatingBuffer.this.crc.update(GzipInflatingBuffer.this.inflaterInput, GzipInflatingBuffer.this.inflaterInputStart, access$000);
                GzipInflatingBuffer.access$112(GzipInflatingBuffer.this, access$000);
                access$000 = i - access$000;
            } else {
                access$000 = i;
            }
            if (access$000 > 0) {
                ...
                int i2 = 0;
                while (i2 < access$000) {
                    int min = Math.min(access$000 - i2, bArr.length);
                    GzipInflatingBuffer.this.gzippedData.readBytes(bArr, 0, min);
                    GzipInflatingBuffer.this.crc.update(bArr, 0, min);
                    i2 += min;
                }
            }
    ...
    boolean isStalled() {
        Preconditions.checkState(this.closed ^ 1, "GzipInflatingBuffer is closed");
    ...
    int inflateBytes(byte[] bArr, int i, int i2) throws DataFormatException, ZipException {
        Preconditions.checkState(this.closed ^ true, "GzipInflatingBuffer is closed");
        boolean z = false;
        boolean z2 = true;
        int i3 = 0;
    ...
    private boolean initializeInflater() {
        ...
        i -= i2;
        if (i > 0) {
            this.inflater.setInput(this.inflaterInput, i2, i);
    ...
    private int inflate(byte[] bArr, int i, int i2) throws DataFormatException, ZipException {
        ...
        try {
            ...
            i2 = this.inflater.inflate(bArr, i, i2);
