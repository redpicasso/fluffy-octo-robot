...
class Headers {
    ...
    public static List<Header> createRequestHeaders(Metadata metadata, String str, String str2, String str3, boolean z) {
        ...
        List<Header> arrayList = new ArrayList(InternalMetadata.headerCount(metadata) + 7);
        ...
        byte[][] toHttp2Headers = TransportFrameUtil.toHttp2Headers(metadata);
        for (int i = 0; i < toHttp2Headers.length; i += 2) {
            ByteString of = ByteString.of(toHttp2Headers[i]);
            if (isApplicationHeader(of.utf8())) {
                arrayList.add(new Header(of, ByteString.of(toHttp2Headers[i + 1])));
            }
        }
        return arrayList;
    }
