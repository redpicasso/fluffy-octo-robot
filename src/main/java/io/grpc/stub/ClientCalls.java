...
public final class ClientCalls {
    ...
    private static final class BlockingResponseStream<T> implements Iterator<T> {
        ...
        private final class QueuingListener extends Listener<T> {
            ...
            public void onMessage(T t) {
                Preconditions.checkState(this.done ^ 1, "ClientCall already closed");
        ...
        public boolean hasNext() {
            ...
            BlockingResponseStream blockingResponseStream = this.last;
            if (!(blockingResponseStream instanceof StatusRuntimeException)) {
                return blockingResponseStream != this;
            } else {
                StatusRuntimeException statusRuntimeException = (StatusRuntimeException) blockingResponseStream;
    ...
    private static final class GrpcFuture<RespT> extends AbstractFuture<RespT> {
        ...
        protected String pendingToString() {
            return MoreObjects.toStringHelper((Object) this).add("clientCall", this.call).toString();
        }
    }
    ...
    public static <ReqT, RespT> RespT blockingUnaryCall(ClientCall<ReqT, RespT> clientCall, ReqT reqT) {
        ClientCall clientCall2;
        try {
            clientCall2 = getUnchecked(futureUnaryCall(clientCall2, reqT));
            return clientCall2;
        } catch (Throwable e) {
            throw cancelThrow(clientCall2, e);
        } catch (Throwable e2) {
            throw cancelThrow(clientCall2, e2);
        }
    }
    ...
    public static <ReqT, RespT> RespT blockingUnaryCall(Channel channel, MethodDescriptor<ReqT, RespT> methodDescriptor, CallOptions callOptions, ReqT reqT) {
        Object threadlessExecutor = new ThreadlessExecutor();
        ClientCall newCall = channel.newCall(methodDescriptor, callOptions.withExecutor(threadlessExecutor));
        try {
            Future futureUnaryCall = futureUnaryCall(newCall, reqT);
    ...
    public static <ReqT, RespT> Iterator<RespT> blockingServerStreamingCall(Channel channel, MethodDescriptor<ReqT, RespT> methodDescriptor, CallOptions callOptions, ReqT reqT) {
        Object threadlessExecutor = new ThreadlessExecutor();
        ClientCall newCall = channel.newCall(methodDescriptor, callOptions.withExecutor(threadlessExecutor));
        Iterator blockingResponseStream = new BlockingResponseStream(newCall, threadlessExecutor);
    ...
    public static <ReqT, RespT> ListenableFuture<RespT> futureUnaryCall(ClientCall<ReqT, RespT> clientCall, ReqT reqT) {
        ListenableFuture grpcFuture = new GrpcFuture(clientCall);
        asyncUnaryRequestCall((ClientCall) clientCall, (Object) reqT, new UnaryStreamToFuture(grpcFuture), false);
        return grpcFuture;
    }
    ...
    private static <ReqT, RespT> void asyncUnaryRequestCall(ClientCall<ReqT, RespT> clientCall, ReqT reqT, StreamObserver<RespT> streamObserver, boolean z) {
        asyncUnaryRequestCall((ClientCall) clientCall, (Object) reqT, new StreamObserverToCallListenerAdapter(streamObserver, new CallToStreamObserverAdapter(clientCall), z), z);
    }
    ...
    private static <ReqT, RespT> StreamObserver<ReqT> asyncStreamingRequestCall(ClientCall<ReqT, RespT> clientCall, StreamObserver<RespT> streamObserver, boolean z) {
        StreamObserver callToStreamObserverAdapter = new CallToStreamObserverAdapter(clientCall);
